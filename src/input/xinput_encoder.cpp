...
std::vector<uint8_t> XInputEncoder::encode(const GamepadState& state) {
    ...
    constexpr int TOTAL_SIZE = HEADER_SIZE + 1 + FRAME_SIZE;
    ...
    buf[off++] = static_cast<uint8_t>(seq & 0xFF);
    buf[off++] = static_cast<uint8_t>((seq >> 8) & 0xFF);
    buf[off++] = static_cast<uint8_t>((seq >> 16) & 0xFF);
    buf[off++] = static_cast<uint8_t>((seq >> 24) & 0xFF);

    write_timestamp(buf, off);

    // --- Gamepad section ---
    buf[off++] = 1;  // frame count
    ...
    int16_t lx = norm_axis(state.left_stick_x);
    int16_t ly = norm_axis(state.left_stick_y);
    ...
    int16_t rx = norm_axis(state.right_stick_x);
    int16_t ry = norm_axis(state.right_stick_y);
    ...
    if (std::abs(state.left_stick_x) > DEADZONE || std::abs(state.left_stick_y) > DEADZONE) {
        physicality |= (PhysLeftThumbX | PhysLeftThumbY);
    }
    if (std::abs(state.right_stick_x) > DEADZONE || std::abs(state.right_stick_y) > DEADZONE) {
        physicality |= (PhysRightThumbX | PhysRightThumbY);
    }
    if (state.left_trigger > 0) {
        physicality |= PhysLeftTrigger;
    }
    if (state.right_trigger > 0) {
        physicality |= PhysRightTrigger;
    }
...
std::vector<uint8_t> XInputEncoder::encodeMetadata(uint8_t max_touch_points) {
