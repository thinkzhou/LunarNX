...
#include "session/channels/video/frame_hevc.h"

#include "ihs_media_metadata.h"
static _Thread_local uint32_t lunar_video_timestamp;
uint32_t LunarIHSVideoTimestamp(void) { return lunar_video_timestamp; }
...
static bool AssembleFrame(IHS_SessionChannel *channel) {
    ...
    while (partial != NULL && !videoCh->states.frameFinished) {
        ...
        if (videoCh->frame.buffer.size == 0) videoCh->frame.timestamp = partial->timestamp;
        // append buffer
...
static void SubmitFrame(IHS_SessionChannel *channel, IHS_Buffer *data, IHS_StreamVideoFrameFlag flags) {
    ...
    lunar_video_timestamp = ((IHS_SessionChannelVideo *) channel)->frame.timestamp;
