...
void IHS_SessionChannelAdd(IHS_Session *session, IHS_SessionChannel *channel) {
    ...
    if (existing || session->base.interrupted ||
...
void IHS_SessionChannelRemove(IHS_Session *session, IHS_SessionChannelId channelId) {
    ...
    LunarIHSTimerUnlock(session->timers);
    // Disconnect iterates under the timer lock. Detach before destroying, and
    // release it before joining a media worker whose stop callback also uses timers.
