...
static void OnNegotiationInit(IHS_SessionChannel *channel, const CNegotiationInitMsg *message, uint16_t packetId) {
    ...
    // LunarNX sends gamepad reports through Remote HID, not legacy controller messages.
    const bool remoteHid = session->hidManager->providers.size > 0 &&
    ...
    PROTOBUF_C_SET_VALUE(clientConfig, maximum_resolution_x, 1280);
    PROTOBUF_C_SET_VALUE(clientConfig, maximum_resolution_y, 720);
