...
typedef struct {
    ...
    bool disconnectInitiated; // protected by session timer mutex
} DiscoveryChannel;
...
void IHS_SessionChannelDiscoveryDisconnect(IHS_SessionChannel *channel) {
    ...
    IHS_Timer *timer = channel->session->timers;
    LunarIHSTimerLock(timer);
    // Covers protocol-originated calls too, and stays latched after task end.
    if (!discoveryCh->disconnectInitiated) {
        discoveryCh->disconnectInitiated = true;
        discoveryCh->disconnectTimerTask = LunarIHSTimerStartLocked(
            timer, DisconnectTimerRun, DisconnectTimerEnd, 0, channel);
    }
    LunarIHSTimerUnlock(timer);
}

static void OnDiscoveryDeinit(IHS_SessionChannel *channel) {
    DiscoveryChannel *discoveryCh = (DiscoveryChannel *) channel;
    IHS_Timer *timer = channel->session->timers;
    // Drain under the same mutex as task end: never load a stale task pointer.
    LunarIHSTimerLock(timer);
    if (discoveryCh->disconnectTimerTask)
        LunarIHSTimerRemoveLocked(discoveryCh->disconnectTimerTask);
    LunarIHSTimerUnlock(timer);
}
