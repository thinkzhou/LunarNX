...
static _Thread_local uint16_t lunar_audio_sequence;
static _Thread_local uint32_t lunar_audio_timestamp;
uint16_t LunarIHSAudioSequence(void) { return lunar_audio_sequence; }
uint32_t LunarIHSAudioTimestamp(void) { return lunar_audio_timestamp; }
...
static void DataReceived(IHS_SessionChannel *channel, const IHS_SessionDataFrameHeader *header, IHS_Buffer *body) {
    ...
    lunar_audio_sequence = header->id;
    lunar_audio_timestamp = header->timestamp;
