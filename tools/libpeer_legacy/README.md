...
every datagram. The completed-state receive loop uses 128 raw UDP datagrams and
64 RTP packets per pass, or one millisecond, whichever comes first. LunarNX
reports receive/decode work so it can perform a bounded multi-pass drain
without starving DTLS or SCTP; the socket and drain changes are safe to ignore
when a target clamps the requested buffer size. Home and Cloud jitter/recovery
policies are selected above the libpeer pump; the pump remains at the known-
good common budget for both profiles.
