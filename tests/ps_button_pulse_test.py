...
def main():
    ...
    pulse_pos = controller.index("state.guide = true;")
    route_pos = controller.index("state = input_router_.route(state);")
    require(pulse_pos < route_pos,
            "input ownership must be the final authority over a requested PS pulse")
